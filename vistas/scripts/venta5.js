...
if(cont == 0){
	...
	/*bootbox.confirm("¿Desea imprimir el presupuesto?", function(result){
		if (result) {
...
function agregarDetalle(){
	...
	if(codigo === ""){
		...
		document.getElementsByName("codigon").focus();
	}
else {
...
//precios = '<datalist id="pre[]"><option value="'+precio1+'" selected="selected"  >'+precio1+'</option><option value="'+precio2+'">'+precio2+'</option><option value="'+precio3+'">'+precio3+'</option><option value="'+precio4+'">'+precio4+'</option></datalist>';
        ...
        '<td><input type="number" step="any" onkeyup="validarcant(event)"  name="cantidad[]" id="cantidad[]" value="'+cantidad+'"></td>'+
         '<td style="width:15;"><input type="text" onkeyup="cargarPrecios('+idarticulo+')"  name="precio_venta[]" id="precio_venta_'+idarticulo+'" class="form-control" value="'+precio1+'" list="pre[]"><datalist id="pre[]"><option value="'+precio1+'" selected="selected"  ><option value="'+precio2+'"><option value="'+precio3+'"><option value="'+precio4+'"></datalist></td>'+
...
var combo =document.getElementsByName("precio_venta[]");
var cant=document.getElementsByName("cantidad[]");
...
document.getElementsByName("cantidad[]")[i].focus();

}

	//var algo = document.getElementsByName("cantidad[]")[valor];
	//alert(algo);
...
function validarcant(e) {
	...
	//$("#codpro").val("");
	  tecla = (document.all) ? e.keyCode : e.which;
  if (tecla==13) {document.getElementById("codigon").focus();}
  else{ modificarSubtotales(); }
}
...
//alert(precio);
	...
	$.post("../ajax/venta.php?op=buscarProd", {cod: codigo}, function(e){
		...
		alert("El precio de "+data.idarticulo+" es:"+pre);
	}
	...
	var lp=document.getElementsByName("precio_venta[]");
		...
		for(l=0; l<prods.length; l++){
			alert("el valor que tienepre es"+pre+"en la posicion"+l);
		//var inpP=prev[i];
		//$('#precio_venta')[i].append(pre);
		if(prods[l] == data.idarticulo){
		lp[l].value = pre;//= pre.toString();
		alert("nuevo precio de"+ prods[l]+"es"+lp[l].value);
		}
		}
	});
