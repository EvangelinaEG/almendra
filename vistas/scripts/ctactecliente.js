...
$("#codigon").autocomplete({ 
    source: "../ajax/ventas.php?op=getProductos"+$("#codigon").val(),
        min_length: 1, 
   // delay: 300 
}); 
...
function eliminar(id){
	bootbox.confirm("¿Esta seguro de eliminar el pago?", function(result){
		if (result) {
			...
			$.post("../ajax/cccliente.php?op=eliminar", {idcccliente : id }, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function registrarPago(){
	...
	var cli = document.formulario.idcliente.value;
	

	$.post("../ajax/cccliente.php?op=registrar",{monto: nuevomonto, cliente: cli},
...
function listar(){
 ...
 var fecha_fin = $("#fecha_fin").val();
		...
		$.post("../ajax/cccliente.php?op=mostrarTotales",	function(dat,status)
		{

		
			data=JSON.parse(dat);
		var sal = (data.saldo).toFixed(2);
...
function listarPagos(idcliente){
var idcliente = $("#idcliente").val();
 ...
 var fecha_fin = $("#fecha_fin").val();
	...
	tabla=$('#tblpagos').dataTable({
		...
		"ajax":
		{
			url:'../ajax/cccliente.php?op=listarPagos&id='+idcliente+'&fi='+fecha_inicio+'&fin='+fecha_fin,
			type: "get",
...
function mostrar(idcliente, saldo){

	$.post("../ajax/cccliente.php?op=mostrar",{cli : idcliente},
		function(data,status)
		{
			data=JSON.parse(data);
			mostrarform(true);
			$("#idcli").html(data.cliente);
			if(saldo>0){
			$("#monto").val(saldo);
		}else{$("#monto").val("");}
			$("#idcliente").val(idcliente);
			listarPagos(idcliente);
/*			
...
function anular(idcliente){
	bootbox.confirm("¿Esta seguro de desactivar este dato?", function(result){
		if (result) {
			$.post("../ajax/venta.php?op=anular", {idcliente : idcliente}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			});
		}
...
function agregarprod(){
	...
	if(nompro == "" || catepro == ""){
		...
		$.post("../ajax/venta.php?op=agregarArticulo", {nom: nompro, cod: codpro, cate: catepro}, function(e){
		data=JSON.parse(e);
		if(data){
