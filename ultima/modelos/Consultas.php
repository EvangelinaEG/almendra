...
public function productosUfecha($fecha_inicio,$fecha_fin){
	$sql="SELECT a.idarticulo, a.nombre, a.descripcion, v.fecha_hora, dv.cantidad as cant, (dv.cantidad * dv.precio_venta - (dv.cantidad * dv.precio_venta)*(dv.descuento*0.01)- (dv.cantidad* dv.costo)) as utilidad FROM detalle_venta dv INNER JOIN articulo a ON dv.idarticulo = a.idarticulo INNER JOIN venta v ON dv.idventa = v.idventa WHERE DATE(v.fecha_hora) BETWEEN '$fecha_inicio' AND '$fecha_fin' GROUP BY dv.idarticulo";
	return ejecutarConsulta($sql);
}
