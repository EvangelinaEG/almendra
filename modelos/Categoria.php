...
$consulta = "SELECT p.nombre as proveedor, p.idpersona, a.idcategoria, a.idarticulo, a.descripcion, a.costo, a.precioventa1, a.precioventa2, a.precioventa3, a.precioventa4, a.idproveedor FROM persona p  INNER JOIN articulo a ON p.idpersona = a.idproveedor WHERE a.idcategoria ='$idcategoria' AND a.idproveedor = '$proveedor'";
}
	$rta = ejecutarConsulta($consulta);
	while ($row = $rta->fetch_assoc()) {
		$id = $row['idarticulo'];
	...
	}else{if($p1>0){$po1 = (($costo * $por1)/100)+ $costo;}else{$po1 = 0;}}


		if($p2>$costo && $costo>0){
		$po2 = ((($p2 - $costo) * 100)/$costo)+$por2;
		
	}else{if($p2>0){$po2 = (($costo * $por2)/100)+ $costo;}else{$po2 = 0;}}


	if($p3>$costo && $costo>0){
		$po3 = ((($p3 - $costo) * 100)/$costo)+$por3;
		
		}else{if($p3>0){$po3 = (($costo * $por3)/100)+ $costo;}else{$po3 = 0;}}


		if($p4>$costo && $costo>0){
		$po4 = ((($p4 - $costo) * 100)/$costo)+$por4;
		
} else{if($p4>0){$po4 = (($costo * $por4)/100)+ $costo;}else{$po4 = 0;}}
	
		if($po1 == 0){$np1 = 0.00;}else{
		$np1 = (($costo * $po1)/100) + $costo;
		}
		if($po2 == 0){$np2 = 0.00;}else{
		$np2 = (($costo * $po2)/100) + $costo;
	}
	if($po3 == 0){$np3 = 0.00;}else{
		$np3 = (($costo * $po3)/100) + $costo;
	}
	if($po4 == 0){$np4 = 0.00;}else{
