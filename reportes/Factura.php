...
function addCadreTVAs($letras, $pago = "")
{
    ...
    $this->SetXY( $r1+9, $y1);
    $this->Cell(10,4, "Saldos.");
    $this->SetFont( "Arial", "", 8);
    $this->SetXY( $r1, $y2 - 12 );// 15 POSICION EN Y DEL PRECIO EN LETRAS
   ...
   // $this->Cell(8,8, "SALDO.");
    ...
    $this->SetXY( $r1+20, $y1+11);
     $this->SetFont( "Arial", "B", 8);
    $this->Cell(10,5, $pago,0, 0, "right");
    //}
}

function addCadreEurosFrancsA($impuesto)
{
    $r1  = $this->w - 70;
    $r2  = $r1 + 60;
    $y1  = $this->h - 40;
...
//                  "qte"     => qte,
//                  "tva"     => code_tva );
function addTVAsA( $impuesto, $iva, $total, $total_venta, $simbolo, $pago )
{
   // $total_venta = $total + $impuesto + $iva;
    $this->SetFont('Arial','',8);
  ...
  //  $igv=$total_venta-$total;
   // $length = $this->GetStringWidth(round($igv,2) );
    $this->Cell( $length, 2, number_format($total, 2,',', ''));
    // mostrando el total
    $this->SetXY( $re, $y1+7 );
    $length = $this->GetStringWidth( $simbolo );
   $this->Cell( $length, 2, $simbolo);
    ...
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);
     $this->Cell( $length, 2, number_format($iva, 2, ',', ''));
   // $length = $this->GetStringWidth($iva);
    $this->SetXY( $re, $y1+13 );
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);
    //$length = $this->GetStringWidth( $total_venta );
    $this->Cell( $length, 2, number_format(round($total_venta,2), 2, ',', ''));
     $this->SetXY( $re, $y1+16 );
    $length = $this->GetStringWidth( $simbolo );
     $this->Cell( $length, 2, $simbolo);
    //$length = $this->GetStringWidth( $pago );
    $this->Cell( $length, 2, number_format($pago, 2, ',', ''));
...
function addTVAs( $impuesto,  $total,  $simbolo, $pago )
{
    $total_venta = $total + $impuesto;
    $this->SetFont('Arial','',8);
   ...
   $this->Cell( $length, 2, $simbolo);
    ...
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);
     $this->Cell( $length, 2, $iva);
    $length = $this->GetStringWidth( $iva );*/
  /*   $this->SetXY( $re, $y1+15 );
    $length = $this->GetStringWidth( $simbolo );
    $this->Cell( $length, 2, $simbolo);
   $length = $this->GetStringWidth( $total_venta );
    $this->Cell( $length, 2, $pago,2);*/
