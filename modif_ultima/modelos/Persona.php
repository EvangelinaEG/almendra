...
public function listarcp($cat){
	$sql="SELECT p.nombre as proveedor, p.idpersona, a.idcategoria, a.idarticulo, a.descripcion, i.idproveedor FROM persona p INNER JOIN ingreso i ON p.idpersona = i.idproveedor INNER JOIN detalle_ingreso di ON i.idingreso = di.idingreso INNER JOIN articulo a ON di.idarticulo = a.idarticulo WHERE a.idcategoria ='$cat' GROUP BY p.idpersona ";
	return ejecutarConsulta($sql);
}
...
public function actualizarCosto($idproveedor, $costo, $categoria){
	$cont =0;
	if($categoria == "TODOS"){
	$consulta = "SELECT p.nombre as proveedor, p.idpersona, a.idcategoria, a.idarticulo, a.descripcion, a.costo, a.precioventa1, a.precioventa2, a.precioventa3, a.precioventa4, i.idproveedor FROM persona p INNER JOIN ingreso i ON p.idpersona = i.idproveedor INNER JOIN detalle_ingreso di ON i.idingreso = di.idingreso INNER JOIN articulo a ON di.idarticulo = a.idarticulo WHERE i.idproveedor = '$idproveedor'";
}else{
	$consulta = "SELECT p.nombre as proveedor, p.idpersona, a.idcategoria, a.idarticulo, a.descripcion, a.costo, a.precioventa1, a.precioventa2, a.precioventa3, a.precioventa4, i.idproveedor FROM persona p INNER JOIN ingreso i ON p.idpersona = i.idproveedor INNER JOIN detalle_ingreso di ON i.idingreso = di.idingreso INNER JOIN articulo a ON di.idarticulo = a.idarticulo WHERE a.idcategoria ='$categoria' AND i.idproveedor = '$idproveedor'";
}
