...
function registrarPago(){
	...
	bootbox.confirm("¿Esta seguro que desea registrar el pago de $"+nuevomonto+" ?", function(result){
		if (result) {
			...
			var idingreso = document.formulario.idingreso.value;
	
	var prov = document.formulario.idproveedor.value;

	$.post("../ajax/ccproveedor.php?op=registrar",{id : idingreso, monto: nuevomonto, proveedor: prov},
		function(data,status)
		{
			bootbox.alert(data);
			listarPagos(idingreso);
...
function listarPagos(idingreso){
	...
	tabla=$('#tblpagos').dataTable({
		...
		"ajax":
		{
			url:'../ajax/ccproveedor.php?op=listarPagos&id='+idingreso,
			type: "get",
...
function mostrar(idingreso, saldo){

	$.post("../ajax/ccproveedor.php?op=mostrar",{idingreso : idingreso},
		function(data,status)
		{
			data=JSON.parse(data);
			mostrarform(true);
			listarPagos(data.idingreso);
			$("#monto").val(saldo);
...
var fechamod = formato(data.fecha);
	 ...
	 fila="<tr><td><strong>CtaCte: </strong>"+data.idcuentacorriente+
			...
			"</tr><tr></td><td><strong>Id Ingreso: </strong>"+data.idingreso+
			"</td><td><strong> Fecha Ingreso: </strong>"+fechamod+
			"</td><td> <strong>Total: $</strong>"+data.total_compra+
			"</td></tr>";
