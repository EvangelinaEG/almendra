...
precios = '<option value="'+precio1+'" selected="selected"  >'+precio1+'</option><option value="'+precio2+'">'+precio2+'</option><option value="'+precio3+'">'+precio3+'</option><option value="'+precio4+'">'+precio4+'</option>';
        ...
        '<td><input type="number" step="any"  onkeypress="validarcant(event)"  name="cantidad[]" id="cantidad[]" value="'+cantidad+'"></td>'+
...
function validarcant(e) {
	...
	//$("#codpro").val("");
	  tecla = (document.all) ? e.keyCode : e.which;
  if (tecla==13) {alert("entro");modificarSubtotales();document.getElementById("codigon").focus();}
  else{ alert("entro aca");modificarSubtotales();}
}
