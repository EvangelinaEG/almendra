...
switch ($_GET["op"]) {
	...
	$data = array();
		...
		$sald = number_format(round($tot_saldo,2),2,',','');

		$tot = number_format(round($total_general,2),2,',','');
		$data = array('saldo' => $sald, 'total' => $tot);
...
<p tyle="color:green;font-weight:bold;"><?php echo $saldo ?> </p> <?php } ?>
    ...
    case 'listar':
		...
		while ($reg=$rspta->fetch_object()) {
			...
			$data[]=array(
            "0"=>'<button class="btn btn-warning btn-xs" onclick="mostrar('.$reg->idcliente.' ,'.$saldo.')"><i class="fa fa-eye"></i></button>',
            "1"=>$reg->cliente,
            //"2"=>number_format(round($reg->suma,2),2,',',''),
            "2" => '',
		...
		$rspta = $articulo->filtrar($valor);
			...
			$cccliente = new Cccliente();
			$cont =0;
				$rspta=$cccliente->listarPagos($idcliente, $fechainicio, $fechafin);
	...
	$url='../reportes/exTicketcli.php?id=';	
		while ($reg=$rspta->fetch_object()) {
            ...
            "2"=>date("d/m/Y H:i:s", strtotime($reg->fechahorapago)),
  			"3"=>($reg->factura)?number_format(round($reg->total_venta, 2),2,'.',''):0,
            "4"=>($reg->pago>0)?number_format(round($reg->pago, 2),2,'.',''):0,
            "5"=>($sa>0)?'<p style="color:green;font-weight: bold;">'.bcdiv($sa, '1', 2).'</p>':'<p style="color:red;font-weight: bold;">'.bcdiv($sa, '1', 2).'</p>',
             "6"=> (($cont>1) && ($reg->factura == 0))?'<a target="_blank" href="'.$url.$idcliente.'&&fecha='.$reg->fechahorapago.'"> <button class="btn btn-info btn-xs"><i class="fa fa-file"></i></button></a>'.' ' .' <button alt="ELiminar pago" class="btn btn-danger btn-xs" onclick="eliminar('.$reg->idcccliente.')"><i class="fa fa-trash"></i></button>':' <a class="btn btn-warning btn-xs" target="_blank" href="'.$urlf.$reg->factura.'&&fecha='.$reg->fechahorapago.'><button class="btn btn-info btn-xs"><i class="fa fa-eye"></i></button></a>',
