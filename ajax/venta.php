...
switch ($_GET["op"]) {
    ...
    case 'listar':
		...
		case 'selectPrecios':
			...
			while ($reg = $rspta->fetch_object()) {
				echo '<option value='.$reg->precioventa1.'>'.number_format($reg->precioventa1, 2, ',', '.').'</option>';
				echo '<option value='.$reg->precioventa2.'>'.number_format($reg->precioventa2, 2, ',', '.').'</option>';
				echo '<option value='.$reg->precioventa3.'>'.number_format($reg->precioventa3, 2, ',', '.').'</option>';
				echo '<option value='.$reg->precioventa4.'>'.number_format($reg->precioventa4, 2, ',', '.').'</option>';
			}
			break;
		case 'selectCliente':
